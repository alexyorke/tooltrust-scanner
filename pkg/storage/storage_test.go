...
func TestStore_Save_RejectsEmptyToolName(t *testing.T) {
	s := openTestStore(t)
	rec := sampleRecord("missing-tool-name")
	rec.ToolName = "   "

	err := s.Save(context.Background(), rec)

	require.Error(t, err)
	assert.Contains(t, err.Error(), "storage: missing tool_name")
}
...
func TestStore_Get_RejectsEmptyToolName(t *testing.T) {
	dir := t.TempDir()
	dbPath := filepath.Join(dir, "tooltrust.db")
	...
	_, err = s.Get(context.Background(), "missing-tool-name")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "storage: missing tool_name")
}
