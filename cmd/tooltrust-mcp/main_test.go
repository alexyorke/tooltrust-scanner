...
func TestHandleScanServer_RejectsQuotedEmptyCommand(t *testing.T) {
	req := mcplib.CallToolRequest{}
	req.Params.Arguments = map[string]any{"command": `""`}
	...
	assert.True(t, result.IsError)
	text := result.Content[0].(mcplib.TextContent).Text
	assert.Contains(t, text, "empty server command")
}

func TestHandleScanServer_RejectsCommandWithNUL(t *testing.T) {
	req := mcplib.CallToolRequest{}
	req.Params.Arguments = map[string]any{"command": "go\x00bad"}
...
func TestHandleScanServer_RejectsArgumentWithNUL(t *testing.T) {
	req := mcplib.CallToolRequest{}
	req.Params.Arguments = map[string]any{"command": "go bad\x00arg"}

	result, err := handleScanServer(context.Background(), req)
	require.NoError(t, err)
	assert.True(t, result.IsError)
	text := result.Content[0].(mcplib.TextContent).Text
	assert.Contains(t, text, "invalid command argument")
}
...
func TestLoadMCPConfig_RejectsArrayServers(t *testing.T) {
	dir := t.TempDir()
	configData := `{"mcpServers":[]}`
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(configData), 0o644))
...
func TestLoadMCPConfig_RejectsTopLevelNull(t *testing.T) {
	dir := t.TempDir()
...
func TestLoadMCPConfig_RejectsTopLevelArray(t *testing.T) {
	dir := t.TempDir()
...
func TestLoadMCPConfig_RejectsBlankServerArg(t *testing.T) {
	dir := t.TempDir()
	configData := `{"mcpServers":{"broken":{"command":"node","args":["   "]}}}`
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(configData), 0o644))
	...
	_, _, err := loadMCPConfig()
	require.Error(t, err)
	assert.Contains(t, err.Error(), `mcpServers["broken"].args[0] must not be empty`)
}

func TestLoadMCPConfig_RejectsInvalidServerEnvName(t *testing.T) {
	dir := t.TempDir()
	configData := "{\"mcpServers\":{\"broken\":{\"command\":\"node\",\"env\":{\" BAD \":\"x\"}}}}"
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(configData), 0o644))
...
func TestLoadMCPConfig_RejectsServerNameWithNUL(t *testing.T) {
	dir := t.TempDir()
	configData := "{\"mcpServers\":{\"bad\\u0000name\":{\"command\":\"node\"}}}"
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(configData), 0o644))
	...
	_, _, err := loadMCPConfig()
	require.Error(t, err)
	assert.Contains(t, err.Error(), `mcpServers["bad`)
...
func TestScanOneServer_RejectsQuotedEmptyCommand(t *testing.T) {
	result := scanOneServer(context.Background(), "bad-server", mcpServerEntry{
		Command: `""`,
	})
...
func TestScanOneServer_RejectsWhitespaceWrappedServerName(t *testing.T) {
	serverDir := createTempEmptyMCPServer(t)

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	result := scanOneServer(ctx, " foo ", mcpServerEntry{
		Command: "go",
		Args:    []string{"run", serverDir},
	})
...
func TestScanOneServer_RejectsArgWithNUL(t *testing.T) {
	result := scanOneServer(context.Background(), "arg-server", mcpServerEntry{
		Command: "go",
