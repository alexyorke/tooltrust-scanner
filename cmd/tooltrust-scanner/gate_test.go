...
func TestInstallViaConfig_RejectsTopLevelNullConfig(t *testing.T) {
	dir := t.TempDir()
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte("null"), 0o644))
...
func TestInstallViaConfig_RejectsArrayServers(t *testing.T) {
	dir := t.TempDir()
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "failed to parse existing mcpServers in .mcp.json: mcpServers must be an object")
	assert.NotContains(t, err.Error(), "cannot unmarshal")
}

func TestInstallViaConfig_RejectsArrayServerEntry(t *testing.T) {
	dir := t.TempDir()
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(`{"mcpServers":{"broken":[]}}`), 0o644))
...
func TestInstallViaConfig_RejectsNullServerArgs(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(`{"mcpServers":{"broken":{"command":"node","args":null}}}`), 0o644))
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), `failed to parse existing mcpServers in .mcp.json: mcpServers["broken"].args must be an array of strings`)
}

func TestInstallViaConfig_RejectsNonStringServerArg(t *testing.T) {
	dir := t.TempDir()
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(`{"mcpServers":{"broken":{"command":"node","args":[1]}}}`), 0o644))
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), `failed to parse existing mcpServers in .mcp.json: mcpServers["broken"].args[0] must be a string`)
}

func TestInstallViaConfig_RejectsNullServerEnv(t *testing.T) {
	dir := t.TempDir()
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(`{"mcpServers":{"broken":{"command":"node","env":null}}}`), 0o644))
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), `failed to parse existing mcpServers in .mcp.json: mcpServers["broken"].env must be an object`)
}

func TestInstallViaConfig_RejectsNonStringServerEnvValue(t *testing.T) {
	dir := t.TempDir()
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(`{"mcpServers":{"broken":{"command":"node","env":{"PORT":3000}}}}`), 0o644))
...
func TestInstallViaConfig_RejectsBlankServerName(t *testing.T) {
	dir := t.TempDir()
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(`{"mcpServers":{"   ":{"command":"node"}}}`), 0o644))
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), `failed to parse existing mcpServers in .mcp.json: mcpServers contains an empty server name`)
}

func TestInstallViaConfig_RejectsServerNameWithWhitespace(t *testing.T) {
	dir := t.TempDir()
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte(`{"mcpServers":{" bad ":{"command":"node"}}}`), 0o644))
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), `failed to parse existing mcpServers in .mcp.json: mcpServers[" bad "] has invalid surrounding whitespace`)
}

func TestInstallViaConfig_RejectsServerNameWithNUL(t *testing.T) {
	dir := t.TempDir()
	...
	require.NoError(t, os.WriteFile(filepath.Join(dir, ".mcp.json"), []byte("{\"mcpServers\":{\"bad\\u0000name\":{\"command\":\"node\"}}}"), 0o644))
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), `failed to parse existing mcpServers in .mcp.json: mcpServers["bad`)
	assert.Contains(t, err.Error(), `has an invalid server name`)
}
